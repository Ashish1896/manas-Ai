...
export class JournalStorageManager {
        ...
        (entry: any): JournalEntry => ({
          ...
          ai_mood_summary: entry.ai_mood_summary
            ? {
                ...
                generated_at: new Date(entry.ai_mood_summary.generated_at),
              }
            : undefined,
          // Ensure required fields have defaults
          images: entry.images || [],
          stickers: entry.stickers || [],
          template_type: entry.template_type || "cute",
          word_count:
            entry.word_count ||
            this.calculateWordCount(entry.journal_text || ""),
        })
      );
    } catch (error) {
   ...
   */
  private validateEntry(entry: any): entry is JournalEntry {
    return (
      entry &&
      typeof entry.id === "string" &&
      typeof entry.date === "string" &&
      typeof entry.journal_text === "string" &&
      Array.isArray(entry.images) &&
      Array.isArray(entry.stickers)
    );
  }
